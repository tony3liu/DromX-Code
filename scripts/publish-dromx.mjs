...
// If an npm install fails (often the default registry is slow/unreachable in some
// regions), it retries once against a mirror (npmmirror.com) before giving up.
...
function install(src, extraEnv) {
	return spawnSync(process.execPath, [cli, "install", src, "--no-approve"], {
		cwd: here,
		stdio: "ignore",
...
for (const src of sources) {
	let r = install(src);
	// Retry npm sources against the mirror if the default registry failed.
	if (r.status !== 0 && src.startsWith("npm:")) {
		r = install(src, { NPM_CONFIG_REGISTRY: MIRROR, npm_config_registry: MIRROR });
		if (r.status === 0) {
			console.log("  \\u2713 " + src + " (via mirror)");
			continue;
		}
	}
	if (r.status === 0) {
...
if (failed.length) {
	console.log("\\nDromX: some extensions were skipped. Retry later with:");
	for (const src of failed) console.log("  dromx install " + src);
	console.log("If npm is slow, first: npm config set registry " + MIRROR);
}
